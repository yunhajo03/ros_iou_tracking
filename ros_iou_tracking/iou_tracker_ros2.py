...
from vision_msgs.msg import BoundingBox2D, Detection2DArray, Detection2D, Pose2D, ObjectHypothesisWithPose, ObjectHypothesis
from sensor_msgs.msg import Image
from cv_bridge import CvBridge, CvBridgeError
import copy
import numpy as np
import cv2
from ros_iou_tracking.sort import Sort
from ros_iou_tracking.sort import associate_detections_to_trackers
...
class IoUTracker(Node):
    ...
    def __publish_tracking_image(self, image, bboxes):
        ...
        if len(image.shape) <= 2:
            return
        for x in Xs:
            xmin = x.bbox.center.position.x - x.bbox.size_x / 2
            xmax = x.bbox.center.position.x + x.bbox.size_x / 2
            ymin = x.bbox.center.position.y - x.bbox.size_y / 2
            ymax = x.bbox.center.position.y + x.bbox.size_y / 2
    ...
    def __raw_image_callback(self, data):
        ...
        if len(self.new_bboxes) != 0:
            ...
            for bbox in new_bboxes:
                xmin = bbox.bbox.center.position.x - bbox.bbox.size_x / 2
                xmax = bbox.bbox.center.position.x + bbox.bbox.size_x / 2
                ymin = bbox.bbox.center.position.y - bbox.bbox.size_y / 2
                ymax = bbox.bbox.center.position.y + bbox.bbox.size_y / 2
                probability = bbox.results[0].hypothesis.score
        ...
        if trackers.shape[0] != 0:
            for tracker in trackers:
                ...
                detect_msg.bbox.center.position.x = float(center[0])
                detect_msg.bbox.center.position.y = float(center[1])
                detect_msg.bbox.size_x = float(xmax - xmin)
                detect_msg.bbox.size_y = float(ymax - ymin)
        ...
        if matched.shape[0] != 0:
            for pair in matched:
                ...
                self.bboxes[new_id].results[0].hypothesis.score = original_bbox.results[0].hypothesis.score
